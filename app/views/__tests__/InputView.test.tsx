...
describe('InputView', () => {
  ...
  it('renders correctly in manual mode by default', () => {
    ...
    expect(screen.getByPlaceholderText('Ou collez une recette ici...')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Cuisiner/i })).toBeInTheDocument();
  });

  it('handles raw text input change', () => {
    render(<InputView {...defaultProps} />);
    const textarea = screen.getByPlaceholderText('Ou collez une recette ici...') as HTMLTextAreaElement;
  ...
  it('enables "Cuisiner" button if rawText is not empty', () => {
    ...
    expect(screen.getByRole('button', { name: /Cuisiner/i })).toBeEnabled();
      ...
      it('toggles dark/light mode when button is clicked', () => {
        ...
        const toggleButton = screen.getByRole('button', { name: /Passer en mode sombre/i }); // Button has 'Passer en mode sombre' aria-label in light mode
  ...
  it('switches to Gemini tab and handles input', async () => {
    ...
    const geminiTextarea = screen.getByPlaceholderText(/Décrivez votre recette de rêve/i) as HTMLTextAreaElement;
    ...
    expect(defaultProps.handleGeminiGenerate).toHaveBeenCalledWith('My dream recipe');
  });

  it('displays Mealie recipes and loads one on click', async () => {
    const mealieRecipes = [
      { id: '1', slug: 'recipe-1', name: 'Recipe One', description: 'Desc One' },
      { id: '2', slug: 'recipe-2', name: 'Recipe Two', description: 'Desc Two' },
    ];
    mockUseCookingState.mockReturnValue({ ...defaultProps, mealieRecipes, filteredRecipes: mealieRecipes, isMealieLoading: false, activeTab: 'mealie' });

    render(<InputView {...defaultProps} mealieRecipes={mealieRecipes} filteredRecipes={mealieRecipes} activeTab="mealie"/>);
  ...
  it('displays loading state for Mealie recipes', () => {
    mockUseCookingState.mockReturnValue({ ...defaultProps, isMealieLoading: true, activeTab: 'mealie' });
    render(<InputView {...defaultProps} isMealieLoading={true} activeTab="mealie"/>);
    fireEvent.click(screen.getByRole('button', { name: /Mealie/i }));
    expect(screen.getByText('Chargement...')).toBeInTheDocument();
  });

  it('displays error state for Mealie recipes', () => {
    mockUseCookingState.mockReturnValue({ ...defaultProps, mealieError: 'Failed to load', activeTab: 'mealie' });
    render(<InputView {...defaultProps} mealieError="Failed to load" activeTab="mealie"/>);
    fireEvent.click(screen.getByRole('button', { name: /Mealie/i }));
