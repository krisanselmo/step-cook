...
interface CookingViewProps {
  view: UseCookingState['view'];
  setView: UseCookingState['setView'];
  recipe: UseCookingState['recipe'];
  currentStep: UseCookingState['currentStep'];
  setCurrentStep: UseCookingState['setCurrentStep'];
  timer: UseCookingState['timer'];
  setTimer: UseCookingState['setTimer'];
  isTimerRunning: UseCookingState['isTimerRunning'];
  setIsTimerRunning: UseCookingState['setIsTimerRunning'];
  currentTime: UseCookingState['currentTime'];
  ...
  modalOpen: UseCookingState['modalOpen'];
  setModalOpen: UseCookingState['setModalOpen'];
  modalData: UseCookingState['modalData'];
  setModalData: UseCookingState['setModalData'];
  cookedModalOpen: UseCookingState['cookedModalOpen'];
  setCookedModalOpen: UseCookingState['setCookedModalOpen'];
  selectedImage: UseCookingState['selectedImage'];
  setSelectedImage: UseCookingState['setSelectedImage'];
  previewUrl: UseCookingState['previewUrl'];
  setPreviewUrl: UseCookingState['setPreviewUrl'];
  isUploading: UseCookingState['isUploading'];
  setIsUploading: UseCookingState['setIsUploading'];
  uploadSuccess: UseCookingState['uploadSuccess'];
  setUploadSuccess: UseCookingState['setUploadSuccess'];
  stepParams: UseCookingState['stepParams'];
  stepIngredients: UseCookingState['stepIngredients'];
  checkedIngredients: UseCookingState['checkedIngredients'];
  setCheckedIngredients: UseCookingState['setCheckedIngredients'];
  isGeminiMode: UseCookingState['isGeminiMode'];
  setIsGeminiMode: UseCookingState['setIsGeminiMode'];
  fileInputRef: UseCookingState['fileInputRef'];
  t: UseCookingState['t'];
  openMealiePage: UseCookingState['openMealiePage'];
  formatTime: UseCookingState['formatTime'];
  openGeminiModal: UseCookingState['openGeminiModal'];
  handleIngredientAction: UseCookingState['handleIngredientAction'];
  handleFileChange: UseCookingState['handleFileChange'];
  handleUpload: UseCookingState['handleUpload'];
}
...
export const CookingView: React.FC<CookingViewProps> = ({
  ...
  return (
    <div className={`h-screen w-full ${theme.properties.font} flex flex-col overflow-hidden transition-colors duration-300 ${t(theme.colors.rootBgDark, theme.colors.rootBgLight)}`}>
      {/* Header */}
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        {isOverview ? (
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <h2 className={`text-2xl font-bold ${theme.colors.accent}`}>Ingrédients</h2>
                  ...
                  <button
                    ...
                    className={`flex w-full items-center gap-4 text-left p-3 ${theme.properties.radius} transition-all ${
                      isChecked && !isGeminiMode
                        ? t(theme.colors.checkedBgDark, theme.colors.checkedBgLight) + ' line-through'
                        : t('text-gray-300 hover:bg-gray-900/50', 'text-gray-700 hover:bg-white border border-transparent hover:border-gray-200 hover:shadow-sm')
                    }`}
                  >
                    {isGeminiMode ? (
                      ...
                      <button
                        ...
                        className={`px-4 py-2 ${theme.properties.radius} flex items-center gap-2 border transition-all hover:scale-105 active:scale-95 ${
                          isGeminiMode
                            ...
                            : isChecked
                              ? t(theme.colors.checkedBgDark, theme.colors.checkedBgLight) + ' opacity-60 line-through'
                              : t('bg-gray-800/50 border-gray-700 text-gray-300 hover:bg-gray-700', 'bg-white border-gray-200 text-gray-700 shadow-sm hover:bg-gray-50')
                        }`}
                      >
                        {isGeminiMode ? (
                          <Sparkles size={14} className="text-purple-500"/>
                        ) : isChecked ? (
                          <Check size={14} className={theme.colors.accent} strokeWidth={3}/>
                        ) : (
        ...
        {modalOpen && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
            <div className={`w-full max-w-sm p-6 ${theme.properties.radius} shadow-2xl relative ${t('bg-gray-900 border border-gray-700 text-white', 'bg-white border border-gray-200 text-gray-900')}`}>
