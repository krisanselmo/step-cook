...
export async function POST(request: Request) {
  ...
  if (!baseUrl || !token) {
    return NextResponse.json(
      { error: 'Configuration Mealie manquante (URL ou Token)' },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    try {
      ...
      const logResponse = await fetch(
        `${baseUrl}/api/recipes/${slug}/cook-logs`,
        {
          method: 'POST',
          headers: {
            ...commonHeaders,
    ...
    if (image && image.size > 0) {
      ...
      if (!imgResponse.ok) {
        ...
        console.error(
          `[Mealie Image] Upload failed (${imgResponse.status}):`,
          imgErrorText,
        );
    ...
    return NextResponse.json({
      success: true,
      message: 'Cuisiné ! Ajouté à la timeline.',
    });
  } catch (error: unknown) {
