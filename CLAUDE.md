...
├── lib/
│   ├── types.ts               # Interfaces (Recipe, Ingredient, StepParams, Mealie types, ThemePlugin)
...
├── views/
│   ├── InputView.tsx           # Page d'accueil : liste Mealie, mode manuel, mode Gemini
...
Les recettes arrivent par 3 chemins, tous convergent vers le type `Recipe` :

1. **Gemini** : prompt utilisateur → API Gemini → JSON brut → `parseRecipe()` → `Recipe`
